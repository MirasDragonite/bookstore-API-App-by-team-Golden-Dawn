...
func (m UserModel) Insert(user *User) error {
	...
	args := []any{user.Name, user.Email, user.Password.hash, user.Activated, user.Roles}
...
func ValidateUser(v *validator.Validator, user *User) {
	...
	//Проверяет что бы мы писали только либо админ или юзер,в других случях высветят ошибку
	v.Check(user.Roles == "admin" || user.Roles == "user", "roles", "role must be admin or user")
	// v.Check(user.Roles == "user", "roles", "role must be admin or user")
	// v.Check(user.Roles != "", "roles", "must be provided")
...
func (m UserModel) GetForToken(tokenScope, tokenPlaintext string) (*User, error) {
	...
	query := `
	SELECT users.id, users.created_at, users.name, users.email, users.password_hash, users.activated, users.version,users.roles
	FROM users
	INNER JOIN tokens
